 ...
 * @return void
 */
function tops_options_page() {

	$settings_tabs = tops_get_settings_tabs();
	$settings_tabs = empty($settings_tabs) ? array() : $settings_tabs;
	$active_tab    = isset( $_GET['tab'] ) ? sanitize_text_field( $_GET['tab'] ) : 'general';
	$active_tab    = array_key_exists( $active_tab, $settings_tabs ) ? $active_tab : 'general';
	$sections      = tops_get_settings_tab_sections( $active_tab );
	$key           = 'main';
	...
	$registered_sections = tops_get_settings_tab_sections( $active_tab );
	...
	$all_settings = tops_get_registered_settings();
	...
	<div class="wrap <?php echo 'wrap-' . $active_tab; ?>">
		<h2><?php _e( 'Total Product Support Settings', 'total-product-support' ); ?></h2>
		<h2 class="nav-tab-wrapper">
			<?php
			foreach ( tops_get_settings_tabs() as $tab_id => $tab_name ) {
		...
		<div id="tab_container">
			<form method="post" action="options.php">
				...
				settings_fields( 'tops_settings' );

				if ( 'main' === $section ) {
					do_action( 'tops_settings_tab_top', $active_tab );
				}

				do_action( 'tops_settings_tab_top_' . $active_tab . '_' . $section );

				do_settings_sections( 'tops_settings_' . $active_tab . '_' . $section );

				do_action( 'tops_settings_tab_bottom_' . $active_tab . '_' . $section  );

				// For backwards compatibility
				if ( 'main' === $section ) {
					do_action( 'tops_settings_tab_bottom', $active_tab );
				}

				// If the main section was empty and we overrode the view with the next subsection, prepare the section for saving
				if ( true === $override ) {
					?><input type="hidden" name="tops_section_override" value="<?php echo $section; ?>" /><?php
				}
				?>
				</table>
				<?php submit_button(); ?>
